...
const prefixCls = 'dig-upload';
import { Line } from '../progress/progress.jsx';
import classNames from 'classnames';
import FontIcon from '../font-icon/font-icon.jsx';
...
export default React.createClass({
  ...
  render() {
    let list = this.props.items.map(file => {
      let progress;
      let icon = <FontIcon className="bingo-dig-icons-371" />;
      ...
      if (this.props.listType === 'picture' || this.props.listType === 'picture-card') {
        if (file.status === 'uploading' || (!file.thumbUrl && !file.url)) {
          if (this.props.listType === 'picture-card') {
            ...
            icon = <FontIcon className={`${prefixCls}-list-item-thumbnail bingo-dig-icons-332`} type="picture" />;
          }
        } else {
        ...
        <div className={infoUploadingClass} key={file.uid}>
          <div className={`${prefixCls}-list-item-info`}>
               ...
               : <span className={`${prefixCls}-list-item-name`}>{file.name}</span>}
            {
              ...
              ? (
                <span>
                  <a href={file.url} target="_blank" style={{ pointerEvents: file.url ? '' : 'none' }}><FontIcon className="bingo-dig-icons-590" /></a>
                  <FontIcon className="bingo-dig-icons-33" onClick={this.handleClose.bind(this, file)} />
                </span>
              ) : <FontIcon className="bingo-dig-icons-679" onClick={this.handleClose.bind(this, file)} />
            }
          </div>
          { progress }
        </div>
